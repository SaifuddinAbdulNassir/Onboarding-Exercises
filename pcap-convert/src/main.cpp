...
bool parseArgs(int argc, char *argv[], Config &config)
{
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            cout << "Command to run the App: ./build/pcap-convert [--vlan <vlan id>] [-ip-version <4|6>] [--ttl <decrement>] " << "[--dns-addr <address>] [--dns-port <port>]  -i Pcapfiles/<input pcap file> -o Pcapfiles/<output pcap file>  \n";
        }
    }
