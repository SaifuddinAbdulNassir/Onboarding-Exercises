...
From the repository root (`pcap-processor`):
...
```bash
./build/app/pcap-processor --vlan 5 --ip-version 4 --ttl 2 --dns-addr 10.0.0.1 --dns-port 5353 -i data/captures/x11-sample.pcap -o data/captures/x11-sample-filtered.pcap
```
...
To build this project natively, navigate to the project root folder (`pcap-processor`) and run the following commands in your terminal:
